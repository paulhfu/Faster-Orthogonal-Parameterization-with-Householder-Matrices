...
def _fast_hmm_2_explicit(v, stride):
    ...
    for idx in range(1, stride):
        P = torch.empty(Y.shape[:-3] + (k, m, m), device=v.device)
        Y_new = torch.empty(Y.shape[:-1] + (Y.shape[-1] + 1, ), device=v.device)
        W = torch.cat([W, u[..., idx:last:stride, :, :]], dim=-1)
        for i in range(k):
            with torch.cuda.stream(torch.cuda.Stream()):
                P[..., i, :, :] = ID - torch.matmul(u[..., i*stride+idx, :, :],
                                                    v[..., i*stride+idx, :, :].transpose(-1, -2))
                Y_new[..., i, :, :] = torch.cat([torch.matmul(P[..., i, :, :], Y[..., i, :, :]),
                                                 v[..., i*stride+idx, :, :]], dim=-1)
